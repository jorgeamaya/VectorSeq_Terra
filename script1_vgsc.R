...
ASVTable_set1 = ASVTable[,c(2,3)]
ASVTable_set2 = ASVTable[,c(4,5)]
colnames(ASVTable_set1) = c('sequence', ' bimera')
colnames(ASVTable_set2) = c('sequence', ' bimera')
ASVTable = rbind(ASVTable_set1, ASVTable_set2)
str(ASVTable$sequence)
ASVTable$haplen = nchar(ASVTable$sequence)
ASVTable = ASVTable[which(ASVTable$haplen != 0),]
colnames(ASVTable) = c("sequence", "bimera", "haplen")

#-------------------------------------------------------------------------
# Load the second DADA2 output file containing the read count numbers per 
# sample and ASV
#-------------------------------------------------------------------------
seqtab <- read_tsv("seqtab_mixed.tsv")
...
#VGSC2
#-----
ASVTable_NoBimeras <- ASVTable[which(ASVTable$bimera == "False"),]
...
#rownames(nonred_seqtab) <- seqtab$SampleID
...
write.csv(DF3, file= "Results/Filtered_seqtab_132bp_nonred.csv", row.names = T)

# 2. matrix with the list of ASVs filtered and trimmed. 
write.csv(DF4, file= "Results/ASV_ID_and_Sequences.csv", row.names = T)
...
# be done.
write.csv(top2, file= "Results/Top2ASVs_perSample.csv", row.names = T)
...
DF1 <- read.csv("Results/Filtered_seqtab_132bp_nonred.csv")
...
library(ggplot2)
barplot_svg = ggplot(data_filtered, aes(x= variable, y = value)) +
...
png("Results/Number_of_sampled_reads.png")
barplot_svg
dev.off()
